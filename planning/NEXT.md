# Tomorrow's plan — 2026-09-16

Tracked objects, in priority order (per `prompts/00-daily-review.md`).

1. **Deposit-crediting bug — live, unresolved, 25 days.**
   ...
   Last directly confirmed against live Vercel runtime logs on
   2026-09-14 (90 "429" hits in a two-hour window, all 30
   scan-completions `credited:0`); re-pull fresh tomorrow rather than
   carrying the number forward again. Any on-chain USDC deposit
   landing in this window is very likely not being credited. Needs
   Dennis's decision on the RPC provider/rate limit, not another
   nightly re-check.
2. **Process integrity — two asks still open, unanswered in `#nexus`.**
   ...
   (open since 08-19, 27 days; no `nexus` deployment newer than
   2026-08-19T20:13Z as of the last check on 2026-09-14 — this is very
   likely the same root cause blocking a code fix for item 1 from ever
   shipping). (b) Auto-merge for planning PRs (open since 08-09, 37
   days) — less urgent procedurally since the merge-logic fix is
   proven across ten straight clean nights (#192-#201), but still
   unanswered.
3. **Launch readiness** (`marketing/token-launch-checklist.md`): the
   ...
   Uniswap v4 / Robinhood Chain venue — 47 days unpicked since the
...
4. **Rienda M1-M5**: last report 2026-07-31 (spec complete; token +
   ...
   development). Now 46 days stale — keep asking Dennis directly for a
   fresh status.
5. **Health checks — still egress-blocked, 47th consecutive confirmed
   night** on `nexus.vdmnexus.com`, `verify.vdmnexus.com`, and
   `www.vdmnexus.com` (proxy `connect_rejected` on the CONNECT tunnel,
   ...
   checks elsewhere.
6. **Standing blocked items**: #106 (cards-v1 spec, 114 days) —
   merge-or-close decision. #95 (Polymarket agent, 115 days) — blocked
...
Ten consecutive clean nights now under the corrected merge logic:
#192 through #201, most recently #201 (2026-09-14, merged cleanly
despite `mergeable_state: unstable`). This session (2026-09-15) is
