# Tomorrow's plan — 2026-09-26

Tracked objects, in priority order (per `prompts/00-daily-review.md`).

1. **Deposit-crediting bug — live, unresolved, 35 days.**
   `/api/v1/deposits/scan` on production reconfirmed fresh tonight
   (2026-09-25, direct pull against live runtime logs scoped to the
   ...
   Unbroken since 2026-08-21. Any on-chain USDC deposit landing in this
   window is very likely not being credited. Needs Dennis's decision on
...
2. **Process integrity — two asks still open, unanswered in `#nexus`.**
   ...
   deployments (open since 08-19, 37 days; reconfirmed fresh tonight —
   ...
   shipping).
   (b) Auto-merge for planning PRs (open since 08-09, 47 days) — less
   urgent procedurally since the merge-logic fix is proven across
   twenty straight clean nights (#192-#211), but still unanswered.
3. **Launch readiness** (`marketing/token-launch-checklist.md`): the
   ...
   Uniswap v4 / Robinhood Chain venue — 57 days unpicked since the
...
4. **Rienda M1-M5**: last report 2026-07-31 (spec complete; token +
   ...
   development). Now 56 days stale — keep asking Dennis directly for a
   fresh status.
5. **Health checks — still egress-blocked, 57th consecutive confirmed
   night** on `nexus.vdmnexus.com`, `verify.vdmnexus.com`, and
   `www.vdmnexus.com` (proxy CONNECT-tunnel `403` `connect_rejected`,
   ...
   checks elsewhere.
6. **Standing blocked items**: #106 (cards-v1 spec, 124 days) —
   merge-or-close decision. #95 (Polymarket agent, 125 days) — blocked
...
Twenty consecutive clean nights now under the corrected merge logic:
#192 through #211, most recently #211 (2026-09-24, merged cleanly). This
session (2026-09-25) opened and merged its own PR the same way. The
