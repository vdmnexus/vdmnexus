# Tomorrow's plan — 2026-09-19

Tracked objects, in priority order (per `prompts/00-daily-review.md`).

1. **Deposit-crediting bug — live, unresolved, 28 days.**
   `/api/v1/deposits/scan` on production reconfirmed fresh tonight
   (2026-09-18, direct pull against live runtime logs, not carried
   forward): all 30 scan invocations in the last 2h hit
   `RPC getTransaction HTTP 429` on the same stuck signatures and
   completed with `credited:0`. Unbroken since 2026-08-21. Any
...
2. **Process integrity — two asks still open, unanswered in `#nexus`.**
   ...
   deployments (open since 08-19, 30 days; reconfirmed fresh tonight —
   ...
   shipping).
   (b) Auto-merge for planning PRs (open since 08-09, 40 days) — less
   urgent procedurally since the merge-logic fix is proven across
   thirteen straight clean nights (#192-#204), but still unanswered.
3. **Launch readiness** (`marketing/token-launch-checklist.md`): the
   ...
   Uniswap v4 / Robinhood Chain venue — 50 days unpicked since the
...
4. **Rienda M1-M5**: last report 2026-07-31 (spec complete; token +
   ...
   development). Now 49 days stale — keep asking Dennis directly for a
   fresh status.
5. **Health checks — still egress-blocked, 50th consecutive confirmed
...
6. **Standing blocked items**: #106 (cards-v1 spec, 117 days) —
   merge-or-close decision. #95 (Polymarket agent, 118 days) — blocked
...
Thirteen consecutive clean nights now under the corrected merge
logic: #192 through #204, most recently #204 (2026-09-17, merged
cleanly despite `mergeable_state: unstable`). This session
(2026-09-18) opened and merged its own PR the same way. The
...
forward, closing out the two-night carry-forward flagged in
yesterday's plan.
