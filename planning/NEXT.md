...
1. **Deposit-crediting bug — live, unresolved, 34 days.**
   `/api/v1/deposits/scan` on production reconfirmed fresh tonight
   (2026-09-24, direct pull against live runtime logs, not carried
   forward): 14 scan invocations in the last 30 minutes, every one
...
2. **Process integrity — two asks still open, unanswered in `#nexus`.**
   ...
   deployments (open since 08-19, 36 days; reconfirmed fresh tonight —
   ...
   shipping).
   (b) Auto-merge for planning PRs (open since 08-09, 46 days) — less
   urgent procedurally since the merge-logic fix is proven across
   nineteen straight clean nights (#192-#210), but still unanswered.
3. **Launch readiness** (`marketing/token-launch-checklist.md`): the
   ...
   Uniswap v4 / Robinhood Chain venue — 56 days unpicked since the
...
4. **Rienda M1-M5**: last report 2026-07-31 (spec complete; token +
   ...
   development). Now 55 days stale — keep asking Dennis directly for a
   fresh status.
5. **Health checks — still egress-blocked, 56th consecutive confirmed
...
6. **Standing blocked items**: #106 (cards-v1 spec, 123 days) —
   merge-or-close decision. #95 (Polymarket agent, 124 days) — blocked
...
Nineteen consecutive clean nights now under the corrected merge
logic: #192 through #210, most recently #210 (2026-09-23, merged
cleanly despite `mergeable_state: unstable`). This session
(2026-09-24) opened and merged its own PR the same way. The
