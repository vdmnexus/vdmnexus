# Tomorrow's plan — 2026-09-22

Tracked objects, in priority order (per `prompts/00-daily-review.md`).

1. **Deposit-crediting bug — live, unresolved, 31 days.**
   `/api/v1/deposits/scan` on production reconfirmed fresh tonight
   (2026-09-21, direct pull against live runtime logs, not carried
   forward): 30 scan invocations in the last 2h, every one hitting
...
2. **Process integrity — two asks still open, unanswered in `#nexus`.**
   ...
   deployments (open since 08-19, 33 days; reconfirmed fresh tonight —
   ...
   shipping).
   (b) Auto-merge for planning PRs (open since 08-09, 43 days) — less
   urgent procedurally since the merge-logic fix is proven across
   sixteen straight clean nights (#192-#207), but still unanswered.
3. **Launch readiness** (`marketing/token-launch-checklist.md`): the
   ...
   Uniswap v4 / Robinhood Chain venue — 53 days unpicked since the
...
4. **Rienda M1-M5**: last report 2026-07-31 (spec complete; token +
   ...
   development). Now 52 days stale — keep asking Dennis directly for a
   fresh status.
5. **Health checks — still egress-blocked, 53rd consecutive confirmed
...
6. **Standing blocked items**: #106 (cards-v1 spec, 120 days) —
   merge-or-close decision. #95 (Polymarket agent, 121 days) — blocked
...
Sixteen consecutive clean nights now under the corrected merge
logic: #192 through #207, most recently #207 (2026-09-20, merged
cleanly despite `mergeable_state: unstable`). This session
(2026-09-21) opened and merged its own PR the same way. The
