...
0. **Process integrity — three open decisions, unanswered a full
   month combined.** Tonight (2026-08-30) re-verified the `nexus`
   Vercel-check failure fresh (checked PR #175 and PR #185 directly
   rather than trusting carry-forward): still `Vercel – nexus:
   failure`, unchanged since 2026-08-18T20:13Z — the **13th
   consecutive night**, diagnosed cause unchanged (Hobby-plan
   cron-frequency limit rejecting `apps/nexus/vercel.json`'s
   `*/2 * * * *` deposit-scan cron). Not merging #175-#185 or
   tonight's PR — genuine CI-red, not the historical
   stuck-despite-green pattern, so the never-merge-without-
   confirmed-green-CI policy holds. Backlog is now **twelve** open
   planning PRs. If the check clears before the next session, merge
   in order #175 → ... → #185 → tonight's PR, rather than starting
   fresh. Three decisions remain open in `#nexus`, a channel with
   zero human replies anywhere in its history: (1) Vercel
   plan-vs-cron-cadence for `nexus`, since 08-21 (9 days). (2)
   auto-merge-for-planning-PRs ask, since 08-09 (21 days) — now
   directly responsible for the backlog size. (3) financial stakes —
   the deposit-scan cron is crediting 0 deposits per run (Solana RPC
   429s), on-chain USDC deposits very likely not being credited,
   unaddressed since 08-21 (9 days). Keep repeating all three
   plainly; no further escalation in wording until one gets a reply.
1. **Launch readiness** (`marketing/token-launch-checklist.md`): the
   ...
   36th night running as the top actionable item with no PR picking
...
2. **Rienda M1-M5**: last reported status (2026-07-31, from Dennis) —
   ...
   started. Now 30 days stale as of 2026-08-30 — continue asking
   Dennis directly for a fresh update. Carry forward unchanged until
   one lands.
3. **Health checks — still an infra blocker, 31 consecutive confirmed
...
4. **Standing blocked items**: #106 (cards-v1 spec) — still a merge-
   or-close decision, open since 2026-05-24 (98 days). #95 (Polymarket
   agent) — still blocked on Spanish counsel, 99 days, now
...
fresh — 13th consecutive night, unchanged since 2026-08-18T20:13Z.
Not merging the twelve-deep backlog (#175-#185 + tonight's PR). Three
...
given the financial stakes of the deposit-crediting item and the
continued silence.
