# Tomorrow's plan — 2026-09-21

Tracked objects, in priority order (per `prompts/00-daily-review.md`).

1. **Deposit-crediting bug — live, unresolved, 30 days.**
   `/api/v1/deposits/scan` on production reconfirmed fresh tonight
   (2026-09-20, direct pull against live runtime logs, not carried
   forward): 28 scan invocations in the last 2h, every one hitting
...
2. **Process integrity — two asks still open, unanswered in `#nexus`.**
   ...
   deployments (open since 08-19, 32 days; reconfirmed fresh tonight —
   ...
   shipping).
   (b) Auto-merge for planning PRs (open since 08-09, 42 days) — less
   urgent procedurally since the merge-logic fix is proven across
   fifteen straight clean nights (#192-#206), but still unanswered.
3. **Launch readiness** (`marketing/token-launch-checklist.md`): the
   ...
   Uniswap v4 / Robinhood Chain venue — 52 days unpicked since the
...
4. **Rienda M1-M5**: last report 2026-07-31 (spec complete; token +
   ...
   development). Now 51 days stale — keep asking Dennis directly for a
   fresh status.
5. **Health checks — still egress-blocked, 52nd consecutive confirmed
...
6. **Standing blocked items**: #106 (cards-v1 spec, 119 days) —
   merge-or-close decision. #95 (Polymarket agent, 120 days) — blocked
...
Fifteen consecutive clean nights now under the corrected merge
logic: #192 through #206, most recently #206 (2026-09-19, merged
cleanly despite `mergeable_state: unstable`). This session
(2026-09-20) opened and merged its own PR the same way. The
