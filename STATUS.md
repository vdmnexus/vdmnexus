...
**#192 through #208 (2026-09-05 through 2026-09-21 daily reviews) —
seventeen consecutive clean nights under the corrected merge logic.**
Each PR opened and merged within the same session (times ranging from
~7 seconds to ~90 seconds), most recently #208 (2026-09-21, merged
`90fe471b`) despite `mergeable_state: unstable` (the `nexus` Vercel
...
2026-09-22 (direct pull against Vercel's `list_teams`/`list_deployments`
...
which hit `RPC getTransaction HTTP 429` on all 15 invocations in the
last 30-minute window, crediting 0 every time. Both remain unresolved
asks in `#nexus` pending a decision from Dennis.
