/**
 * Base mainnet smoke test for the SendAI plugin's EVM path.
 *
 * Same as smoke-base-sepolia.mjs but targets `eip155:8453`. Real USDC
 * gets spent (≈0.02 USDC per call — protocol fee 0.01 + receipt fee 0.01).
 * Real ERC-3009 settlement on Base mainnet.
 *
 * Required env:
 *   TEST_AGENT_PUBKEY     base58 Solana pubkey for the stub SendAI wallet
 *   TEST_EVM_PRIVATE_KEY  0x-prefixed hex secp256k1 secret, funded on
 *                         Base mainnet with USDC (≥0.05) + ETH (any dust)
 */
...
console.log(`Network:    eip155:8453 (Base mainnet — real USDC)`);
console.log();
...
const messages = [
  { role: "user", content: "Reply with exactly: hello from base mainnet" },
];

console.log("→ NEXUS_CHAT (network=eip155:8453) …");
const t0 = Date.now();
...
console.log(`  explorer:        https://basescan.org/tx/${receipt.payment.tx_signature}`);
