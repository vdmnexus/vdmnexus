...
The transaction below is on Solana devnet (so you can inspect it without
spending real USDC). Mainnet works identically — pass
`network: "mainnet"` to `payAndInfer` and the payment lands on Solana
mainnet instead.
