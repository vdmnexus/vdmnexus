...
The diagram is drawn for Solana, but the same flow runs on Base and
Base Sepolia — agents pick per request via `body.network`
(`solana`, `base`, `base-sepolia`). The facilitator routes to the
right chain transparently. SIR v2 §13 covers the chain bindings.
