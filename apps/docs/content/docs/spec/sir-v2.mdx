...
- `prompt_hash` = SHA-256(canonical JSON of `messages`)
- `response_hash` = SHA-256(`choices[0].message.content`)

The canonical JSON encoding is the same one used for the
`nexus_signature` payload (§9): keys sorted lexicographically, no
whitespace, UTF-8. Hashing a delimited string of `${role}:${content}`
would let user content containing a `:` or newline collide with a
structurally different message log; canonical JSON makes the
representation unambiguous.

Verifiers SHOULD also accept receipts whose `prompt_hash` matches the
legacy delimited form (`messages.map(m => "${role}:${content}").join("\n")`)
to remain compatible with receipts signed before 2026-05-23.
