...
> **Status:** Draft · **Spec version:** 1.0 · **Wire format version:** 2
...
| `tx_signature` | string          | Blockchain transaction signature (base58 on Solana).                                          |
| `network`      | string (CAIP-2) | CAIP-2 network identifier. Conformant operators MUST use the genesis-hash form (see §13).     |
| `pay_to`       | string          | Recipient address that received the payment.                                                  |
...
- `agent_pubkey`, `pay_to`, `tx_signature`, and `nexus_signature` MUST be encoded base58 using the Bitcoin alphabet (`123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz`).
- `agent_pubkey` MUST decode to 32 bytes (Ed25519 public key).
- `nexus_signature` MUST decode to 64 bytes (Ed25519 detached signature).
...
The reference chain binding is Solana. The CAIP-2 network identifier
MUST use the genesis-hash form:
...
### 13.2 Future networks (non-normative)

This specification is chain-agnostic at the format level. Future
revisions MAY add binding sections for Base, Ethereum L2s, Bitcoin,
or other networks. Such bindings MUST preserve the verifier's ability
to perform checks 4–5 against the network's authoritative state.
...
- Use the genesis-hash CAIP-2 form for `payment.network` on Solana.
...
Three reference vectors are published alongside this specification:

- **[`prepaid-001/`](/spec/vectors/prepaid-001/receipt.json)** — happy-path prepaid receipt. All five verifier checks pass.
- **[`x402-001/`](/spec/vectors/x402-001/receipt.json)** — happy-path x402 receipt for hashes + signature. On-chain checks are offline-skipped (the `tx_signature` is synthetic).
