...
export function PredictionsPublicView({
  initialBets,
  initialSummary,
}: {
  initialBets: PublicBet[];
  initialSummary: PublicSummary;
}) {
  const [bets, setBets] = useState<PublicBet[]>(initialBets);
  const [summary, setSummary] = useState<PublicSummary>(initialSummary);
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (data.ok && data.bets && data.summary) {
        setBets(data.bets);
...
function SummaryCards({ summary }: { summary: PublicSummary }) {
  return (
...
function BetRow({
  ...
  if (bet.status === "open")
    statusClass = "border-accent-indigo/60 text-accent-indigo";
      ...
      {expanded && (
        <div className="mt-3 space-y-2 rounded-lg border border-soft bg-bg/40 p-4">
          ...
          <p className="text-[10px] text-text-muted">
            model: <code>{bet.model}</code>
            {bet.agent_pubkey_short ? (
              <>
                {" "}· agent: <code>{bet.agent_pubkey_short}</code>
              </>
            ) : null}
          </p>
        </div>
